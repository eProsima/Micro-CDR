...
public:
    ...
    {
        EXPECT_EQ(static_cast<int>(ucdr_used_size(&writer)) % alignment, 0);
        EXPECT_EQ(static_cast<int>(ucdr_used_size(&reader)) % alignment, 0);
    }
...
INSTANTIATE_TEST_CASE_P(Offset, Alignment,
        ::testing::Combine(::testing::Range(size_t(0), size_t(17)), ::testing::Values(UCDR_BIG_ENDIANNESS, UCDR_LITTLE_ENDIANNESS)));
