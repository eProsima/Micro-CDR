...
TEST_F(SequenceOverflow, Block1)
{
    ...
    EXPECT_FALSE(deserialize_sequence_uint8_t(&reader, output, &output_size, ARRAY_LENGTH));
}
...
TEST_F(SequenceOverflow, Block2)
{
    ...
    EXPECT_FALSE(deserialize_sequence_uint16_t(&reader, output, &output_size, ARRAY_LENGTH));
}
...
TEST_F(SequenceOverflow, Block4)
{
    ...
    EXPECT_FALSE(deserialize_sequence_uint32_t(&reader, output, &output_size, ARRAY_LENGTH));
}
...
TEST_F(SequenceOverflow, Block8)
{
    ...
    EXPECT_FALSE(deserialize_sequence_uint64_t(&reader, output, &output_size, ARRAY_LENGTH));
}
